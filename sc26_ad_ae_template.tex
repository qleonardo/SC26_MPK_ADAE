...
For each matrix and each MPK variant, the program runs 100 iterations and reports the average execution time. A batch script (\texttt{run.sh}) is provided to iterate over all matrices; edit the script to set the matrix directory path and NUMA binding. For each matrix, run:
\begin{center}
...
A batch run script is provided. Edit the script to set the matrix directory path and NUMA binding, then execute. For a single matrix (CPU):
\begin{center}
