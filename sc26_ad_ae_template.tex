...
\begin{description}
\item[$C_1$] We present FastMPK, a high-performance sparse matrix power kernel (MPK) that exploits cache reuse across successive SpMVs. FastMPK models the matrix as a dependency graph, recursively partitions it into cache-resident subgraphs, and employs a dependency-preserving runtime scheduler. Evaluated on two x86 CPUs, one ARM CPU, and two NVIDIA GPUs across large SuiteSparse matrices, FastMPK achieves average speedups of up to 1.84$\times$ over state-of-the-art cache-reuse MPK implementations and up to 1.43$\times$ over vendor-tuned libraries.
...
\midrule
Intel Xeon Max 9468 (CPU)  & $\sim$15 \\
AMD EPYC 7742 (CPU)        & $\sim$20 \\
Huawei Kunpeng 920 (CPU)   & $\sim$20 \\
NVIDIA H100 (GPU)          & $\sim$10 \\
NVIDIA A100 (GPU)          & $\sim$15 \\
\bottomrule
\end{tabular}
\end{center}
Times are for the 7 representative matrices with $s\in\{5,10\}$. Actual duration varies with the number of matrices, $s$ values, and available cores.
...
\end{center}
The matrix name is automatically extracted from the file path. Separate scripts are provided for CPU and GPU.
...
Platform & Estimated Time (mins) \\
\midrule
Intel Xeon Max 9468 (CPU) & $\sim$5 \\
NVIDIA H100 (GPU)         & $\sim$2 \\
\bottomrule
\end{tabular}
\end{center}
...
Download from: \url{https://github.com/qleonardo/FastMPK}, or use the Zenodo archive at \url{https://doi.org/10.5281/zenodo.21554566}.
...
The provided \texttt{run.sh} script contains the 7 representative matrices from Table~II. Edit the script to set the matrix directory path and NUMA binding, then execute. For a single matrix (CPU):
\begin{center}
...
Download from: \url{https://github.com/qleonardo/FastMPK-GMRES}, or use the Zenodo archive at \url{https://doi.org/10.5281/zenodo.21554575}.
